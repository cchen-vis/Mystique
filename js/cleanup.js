...
function flattenSVG(svgString) {
    ...
    let rects_processed = temp[0], rectWith0WH = temp[1];
    ...
    if ('text' in groups) {
        ...
        if ('tspan' in groups)
            texts = texts.concat(groups['tspan'])
...
function tree_dict_not_nested(node, level, parent, container, acc_transforms, nodeMap) {
    ...
    if ("transform" in thing) {
        ...
        acc_transforms[0] = parseFloat((parseFloat(dxy[0].substring(1))).toPrecision(6))
        acc_transforms[1] = parseFloat((parseFloat(dxy[1].split(')')[0])).toPrecision(6))
    }
    if (tag=='text') {
        let c = node.outerHTML; //.decode("utf-8") 
        let start = c.indexOf(">"), end = c.indexOf('<', start)
        thing['content'] = ""
        while (end != -1) {
    ...
    if ((node.children.length) > 0 && thing["tag"] != "text") { //avoid parse too low-level to <tspan>
        container.push(thing)
        for (let i = 0; i < node.children.length; i++) {
            let child = node.children[i];
            tree_dict_not_nested(child, level + 1, thing, container, acc_transforms.map(d=>d), nodeMap)
        }
    } else {
        // if tag !='g':
        container.push(thing)
...
function extract_features(container, node, parent, sum_transforms) {
    ...
    if (Object.keys(style).length > 0) {
        if (attrs.indexOf('class') >= 0) {
            ...
            for (let oneClass of thisClass.split(' ')) {
                oneClass = '.'+oneClass
                if (oneClass in style)
                    Object.assign(container, style[oneClass])
            }
        }
    ...
    for (let key of keys_of_interest) {
        if (attrs.indexOf(key) >= 0) {
            if (key == 'style') {
                ...
                let modifiedStyles = []
                for (let pair of allStyles)
                    modifiedStyles.push(pair.trim())
                let style_dict = {};
            ...
            if (key == 'transform') {
                ...
                if (indexOfTranslate != -1) {
                    ...
                    if (pair.indexOf(",") >= 0) {
                        ...
                        while(pair.length != 0 && (pair[pair.length-1]==' ' || pair[pair.length-1]==')'))
                    ...
                    container[key] = "translate(" + String(dx+sum_transforms[0]) + "," + String(dy+sum_transforms[1]) + ")"
                ...
                if (tranString.indexOf("rotate")!=-1)
                    container['rotate'] = true
                if (tranString.indexOf("scale")!=-1)
                    container['scale'] = true
                if (tranString.indexOf("skewX")!=-1)
                    container['skewX'] = true
                if (tranString.indexOf("skewY")!=-1)
                    container['skewY'] = true
            }  else if (key == 'font-size') {
                let v = getAttributeValue(node, key).trim();
                if (v.endsWith("px"))
                    container[key] = parseFloat(v.substring(0, v.length - 2))
                else if (v.endsWith("pt"))
                    container[key] = parseFloat(v.substring(0, v.length - 2)) * 1.3281472327365
                else if (v.endsWith("em"))
                    container[key] = parseFloat(v.substring(0, v.length - 2)) * 16
                else
                    container[key] = parseFloat(v)
            } else if ((key == 'dx' || key == 'dy') && getAttributeValue(node, key).trim().endsWith("em")) {
                // think about this    
    ...
    if (( attrs.indexOf('transform') < 0 || thisTransform) && sum_transforms[0] != 0 && sum_transforms[1] != 0)
        container['transform'] = "translate(" + String(sum_transforms[0]) + "," + String(sum_transforms[1]) + ")"
    return container
}
...
function process_rects(Rects) {
    ...
    let processed = [], rectWith0WH = [];
    for (let rect of target) {
        if (!('width' in rect) || !('height' in rect))
            continue
        if (rect['width'].slice(-1) == '%')
            rect['width'] = parseFloat(rect['width'].substring(0, rect['width'].length-1)) * 25
        if (rect['height'].slice(-1) == '%')
            rect['height'] = parseFloat(rect['height'].substring(0, rect['height'].length-1)) * 25
        if (parseFloat(rect['width'])<=0 && parseFloat(rect['height'])<=0)
            continue
        if (rect['id'].startsWith("path") && !('fill' in rect))
            continue
        ...
        if ('fill' in rect && rect['fill'].indexOf("url") === -1) {
            ...
            if (rect['fill'].indexOf("rgb") > -1 || (rgba_a<=1 && rgba_a > 0) ) {
                ...
                if (rgba[2].indexOf(')') > -1)
                    ...
                    b = rgba[2]
                if ((r.trim() == "100%" || r.trim() == "255") && (g.trim() == "100%" || g.trim() == "255") && (b.trim() == "100%" || b.trim() == "255"))
                    rect['fill'] = "none";
                else 
                    rect["fill"] = (rgba_a<=1 && rgba_a > 0) ? rgbaToHex(parseInt(r), parseInt(g), parseInt(b), rgba_a) : ConvertRGBtoHex(parseInt(r), parseInt(g), parseInt(b));
            }

            rect["fill"] = rect["fill"].startsWith("#") ? rect["fill"].toLowerCase() : rect["fill"];
        }
            
        let dx, dy;
        if ('transform' in rect) {
            let transform_dict = {}, pairs = rect['transform'].split(" ");
        ...
        if ('matrix' in rect) {
            ...
            while (tm.length>0) {
                let oldx = rect['x']
                let oldy = rect['y']
                rect['x'] = parseFloat(tm[0]) * oldx + parseFloat(tm[2]) * oldy + parseFloat(tm[4])
                rect['y'] = parseFloat(tm[1]) * oldx + parseFloat(tm[3]) * oldy + parseFloat(tm[5])
                tm = tm.slice(6)
            }
        }
        ...
        rect['right'] = parseFloat((parseFloat(rect['x'] + rect['width']).toPrecision(6)))
        rect['bottom'] = parseFloat((parseFloat(rect['y'] + rect['height']).toPrecision(6)))
        rect['center'] = parseFloat((parseFloat(rect['x'] + 0.5 * rect['width']).toPrecision(6)))
        rect['middle'] = parseFloat((parseFloat(rect['y'] + 0.5 * rect['height']).toPrecision(6)))
...
function process_circles(Texts) {
    ...
    for (let text of target) {
        ...
        if ('transform' in text) {
            let transform_dict = {}, pairs = text['transform'].split(" ");
            for (let pair of pairs) {
                if (pair != '') {
                    transform_dict[pair.split("(")[0]] = "("+pair.split("(")[1]
                }
            ...
            let dxy = transform_dict['translate'].split(',')
            text['dx'] = text['dx'] + parseFloat(parseFloat(dxy[0].substring(1)).toPrecision(6))
            text['dy'] = text['dy'] + parseFloat(parseFloat(dxy[1].split(')')[0]).toPrecision(6))
        }
        ...
        if ('matrix' in text) {
            ...
            while (tm.length>0) {
                let oldx = text['x']
                let oldy = text['y']
                text['x'] = parseFloat(tm[0]) * oldx + parseFloat(tm[2]) * oldy + parseFloat(tm[4])
                text['y'] = parseFloat(tm[1]) * oldx + parseFloat(tm[3]) * oldy + parseFloat(tm[5])
                tm = tm.slice(6)
            }
        }
    ...
    for (let text of target) {
        ...
        if ('matrix' in text) {
            ...
            while (tm.length>0) {
                let oldx1 = text['x1']
                let oldy1 = text['y1']
                text['x1'] = parseFloat(tm[0]) * oldx1 + parseFloat(tm[2]) * oldy1 + parseFloat(tm[4])
                text['y1'] = parseFloat(tm[1]) * oldx1 + parseFloat(tm[3]) * oldy1 + parseFloat(tm[5])
                let oldx2 = text['x2']
                let oldy2 = text['y2']
                text['x2'] = parseFloat(tm[0]) * oldx2 + parseFloat(tm[2]) * oldy2 + parseFloat(tm[4])
                text['y2'] = parseFloat(tm[1]) * oldx2 + parseFloat(tm[3]) * oldy2 + parseFloat(tm[5])
                tm = tm.slice(6)
            }
        }
        
        if (text['y1'] == text['y2'] && text['x1'] == text['x2'])
            continue
        processed.push(text)
    }
